...
def findBestScore(t):
    ...
    visited = rekFindScore(
        nodesNames.index("AA"), t, 0, nodesDist, nodesFlow, bitwiseVisit
    )

    for visitedMe, valMe in visited.items():
        for visitedElephant, valElephant in visited.items():
            if not (visitedMe & visitedElephant): #only when we didn't visit the same node
                ret = max(valMe + valElephant, ret)
