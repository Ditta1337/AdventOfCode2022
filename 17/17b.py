    ...
    while True:
        ...
        while not isPlaced:
            ...
            if clock == len(moves) - 1:
                clock = -1
            clock += 1
        deletedHight = deleteScreen(screen)
        ret += deletedHight
        if deletedHight > 0:
            deletes.add((deletedHight, rocks, ret))
            
        addToScreen(screen, position)
        if not flag:
            for deletedItem in deletes:     #had to hardcode this part, I dont know how to do it better VVV
                if deletedItem[0] == deletedHight and deletedItem[1] != rocks and rocks - deletedItem[1] > 1200:
                    commonRet = ret - deletedItem[2]
                    commonRocks = rocks - deletedItem[1]
                    ret += commonRet * (((rocksToFall - deletedItem[1]) // commonRocks) - 1)
                    rocks = deletedItem[1] + commonRocks * (((rocksToFall - deletedItem[1]) // commonRocks))
                    flag = 1
                    break
